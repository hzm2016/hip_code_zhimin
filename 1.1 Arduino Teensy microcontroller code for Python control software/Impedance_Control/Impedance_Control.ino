...
Motor_Control_Tmotor sig_m1(Sig_Motor_ID, CAN_ID);
/*MOTOR*/
...
void loop() {
    ...
    // std::memcpy(bytes, &float_value, sizeof(float_value));   
    M1_torque_command = cmd_ampl * sin(2 * M_PI * 1/cmd_fre * t);    
    // sig_m1.sig_torque_cmd(M1_torque_command);   

    l_pos_des = pos_ampl * sin(2 * M_PI * 1/pos_fre * t);    

    Sig_M_Torque_Impedance_Control_Example();  

    // sig_m1.send_sig_cmd(0.0, 0.0, 0.0, 0.0, M1_torque_command);      
    Wait(100);   
}
...
//     M1_torque_command = 0.3;   
//     // M1_torque_command = cmd_ampl * sin(2 * M_PI * 1/cmd_fre * t);   

//     l_pos_des = pos_ampl * sin(2 * M_PI * 1/pos_fre * t);   

//     // M1_Torque_Impedance_Control_Example();  

//     // Sig_M_Torque_Impedance_Control_Example();   
//     // sig_m1.sig_speed_cmd();  
...
void M1_Torque_Impedance_Control_Example(){
    ...
    t_ff = M1_torque_command;   
    tau_imp = (p_des - m1.pos) * kp + (v_des - m1.spe) * kd + t_ff;    

    tau_imp = t_ff; 
    m1.send_cmd(p_des, v_des, kp, kd, tau_imp);   
...
void Sig_M_Torque_Impedance_Control_Example(){
    ...
    tau_imp = t_ff; 
    sig_m1.send_sig_cmd(p_des, v_des, kp, kd, tau_imp);   
...
void M2_Torque_Impedance_Control_Example(){
    ...
    // tau_imp = (p_des - m1.pos) * kp + (v_des - m1.spe) * kd + t_ff;   
    tau_imp = t_ff;   
    m2.send_cmd(p_des, v_des, kp, kd, tau_imp);   
...
void Transmit_ble_Data(){
  ...
  L_vel_int_d = sig_m1.spe * 100;     
  L_vel_int_a = sig_m1.spe * 100;      
